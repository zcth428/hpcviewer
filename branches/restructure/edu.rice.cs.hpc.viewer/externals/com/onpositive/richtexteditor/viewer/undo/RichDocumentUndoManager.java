...
public class RichDocumentUndoManager implements IRichDocumentUndoManager {
	...
	public class UndoableRichTextChange extends AbstractOperation {
		...
		public IStatus undo(IProgressMonitor monitor, IAdaptable uiInfo) {
			if (isValid()) {
				...
				fDocumentUndoManager.fireDocumentUndo(fStart, fPreservedText,
						fText, uiInfo, RichDocumentUndoEvent.UNDONE, false,shouldReveal(false));
				return Status.OK_STATUS;
			}
		...
		protected void redoTextChange(boolean doRichUndo) {
			try {
				...
				manager.getLayer().processChanges(fStart, fEnd - fStart+fText.length(),
		...
		public IStatus redo(IProgressMonitor monitor, IAdaptable uiInfo) {
			if (isValid()) {
				fDocumentUndoManager.fireDocumentUndo(fStart, fText,
						...
						RichDocumentUndoEvent.ABOUT_TO_REDO, false,shouldReveal(true));
				redoTextChange(true);
				fDocumentUndoManager.resetProcessChangeState();
				fDocumentUndoManager.fireDocumentUndo(fStart, fText,
	...
	private class UndoableCompoundTextChange extends UndoableRichTextChange {
		...
		public void fillRedoChanges(ArrayList<CompositeChange>changes){
			...
			for (int a=0;a<fChanges.size();a++){
				UndoableRichTextChange c = (UndoableRichTextChange) fChanges.get(a);
		...
		public IStatus undo(IProgressMonitor monitor, IAdaptable uiInfo) {
			...
			if (size > 0) {
				...
				fDocumentUndoManager.fireDocumentUndo(c.fStart,
						...
						RichDocumentUndoEvent.ABOUT_TO_UNDO, true,shouldReveal(false));
				...
				fDocumentUndoManager.fireDocumentUndo(c.fStart,
						c.fPreservedText, c.fText, uiInfo,
						RichDocumentUndoEvent.UNDONE, true,shouldReveal(false));
			}
		...
		public IStatus redo(IProgressMonitor monitor, IAdaptable uiInfo) {
			...
			if (size > 0) {
				...
				fDocumentUndoManager.fireDocumentUndo(c.fStart, c.fText,
						...
						RichDocumentUndoEvent.ABOUT_TO_REDO, true,shouldReveal(true));
	...
	/**
	 * @see org.eclipse.jface.text.IDocumentUndoManager#transferUndoHistory(
	 * IRichDocumentUndoManager)
	 */
