...
public class ExperimentFileXML extends ExperimentFile {
	 ...
	 * 
	 * 
	 * @deprecated The overload with the stream is more adaptable; use that one when possible.
	 * 
	 * @param experiment
	 *            Experiment object to own the parsed subparts.
	 * @throws Exception
	 ...
	 ************************************************************************/
	  ...
	  public void parse(File file, BaseExperiment experiment, boolean
	  need_metrics, IUserData userData) throws Exception { 
	  ...
	  if (need_metrics) {
		  ...
		  builder = new ExperimentBuilder2(experiment,name, userData); } else { 
			  ...
			  // note: this is a quick hack to fix slow xml reader in ibm bg 
			  // something
	  
	  String callpathLoc = file.getParent() + "/callpath.xml"; File
	  callpathFile = new File(callpathLoc); if (!callpathFile.exists()) {
	  Grep.grep(file.getAbsolutePath(), callpathLoc, "<M ", false);
	  callpathFile = new File(callpathLoc); } stream = new
	  FileInputStream(callpathFile); builder = new
	  BaseExperimentBuilder(experiment, name, userData); }
	  
	  Parser parser = new Parser(name, stream, builder); parser.parse();

	  if (builder.getParseOK() == Builder.PARSER_OK) { // parsing is done successfully 
		  } 
	  else
