...
public class HTMLLexListener implements ILexListener {
	...
	public void handleLexEvent(LexEvent event) {
		if (event instanceof TagLexEvent) {
			...
			if (!tagEvent.isOpen()) {
				int lastIdx = tagsStack.lastIndexOf(tagEvent.type);
				if (lastIdx != -1) {
		...
		} else if (event instanceof TagEndEvent) 
		{
			...
			if (((TagEndEvent) event).isOpen())
			{
				...
				if (((TagEndEvent) event).getType() == Scanner.TYPE_IMG
						&& !curSrc.trim().equals(""))
					// Laks 2009.02.10: add the absolute path of the parent
					appendImage(sParentPath + curSrc);
		...
		for (int i = 0; i < str.length(); i++) {
			if (str.charAt(i) == '&') {
				...
				if (j > i + 1) {
					...
					char chr = CharacterEntityReference
							.getCodePointFromName(link);
