...
/*************************************************************************
 * 
 * SpaceTimeData stores and creates all of the data involved in creating the
 * view including all of the ProcessTimelines.
 * 
 ************************************************************************/
// Eventually I want to make PaintManager extend TraceEvents, not SpaceTimeData,
// and then move all of the references
public class SpaceTimeData  {
	/**
	 * Contains all of the ProcessTimelines. It's a HashMap because, due to the
	 * multithreading, the traces may not get added in order. So, each
	 * ProcessTimeline now knows which line it is, and the HashMap is a map
	 * between that line and the ProcessTimeline.
	 */
	 ...
	 ************************************************************************/
	public SpaceTimeData(IWorkbenchWindow window, File expFile, File traceFile,
	...
	/*
	 * These methods now belong in PaintManager public void setDepth(int _depth)
	 * { this.currentDepth = _depth; }
	 * 
	 * public int getDepth() { return this.currentDepth; }
	 */
	...
	public void paintDetailViewport(final GC masterGC, final GC origGC,
		...
		BaseViewPaint detailPaint = new BaseViewPaint(this, attributes,
				changedBounds, this.statusMgr, window) {

			// @Override
			protected boolean startPainting(int linesToPaint,
					boolean changedBounds) {
				setUpPaintDetailViewport(linesToPaint, changedBounds);
				return true;
			}
	 ...
	 ********************************************************************/
	// This method now belongs in the PaintManager class
	/*
	 * public void paintDepthLine(SpaceTimeSamplePainter spp, int depth, int
	 * height) {
	 * //System.out.println("I'm painting process "+process+" at depth "+depth);
	 * ProcessTimeline ptl = depthTrace;
	 * 
	 * if (ptl.size() < 2) return;
	 * 
	 * double pixelLength = (attributes.endTime -
	 * attributes.begTime)/(double)attributes.numPixelsH; BasePaintLine
	 * depthPaint = new BasePaintLine(colorTable, ptl, spp, attributes.begTime,
	 * depth, height, pixelLength) { //@Override public void finishPaint(int
	 * currSampleMidpoint, int succSampleMidpoint, int currDepth, String
	 * functionName, int sampleCount) { if (currDepth >= depth) {
	 * spp.paintSample(currSampleMidpoint, succSampleMidpoint, height,
	 * functionName); } } };
	 * 
	 * // do the paint depthPaint.paint(); }
	 */
	...
	// This method now belongs in the PaintManager class
	/*
	 * public void paintDetailLine(SpaceTimeSamplePainter spp, int process, int
	 * height, boolean changedBounds) { ProcessTimeline ptl = traces[process];
	 * if (ptl == null || ptl.size()<2 ) return;
	 * 
	 * if (changedBounds) ptl.shiftTimeBy(minBegTime); double pixelLength =
	 * (attributes.endTime - attributes.begTime)/(double)attributes.numPixelsH;
	 * 
	 * // do the paint BasePaintLine detailPaint = new BasePaintLine(colorTable,
	 * ptl, spp, attributes.begTime, currentDepth, height, pixelLength) {
	 * //@Override public void finishPaint(int currSampleMidpoint, int
	 * succSampleMidpoint, int currDepth, String functionName, int sampleCount)
	 * { DetailSpaceTimePainter dstp = (DetailSpaceTimePainter) spp;
	 * dstp.paintSample(currSampleMidpoint, succSampleMidpoint, height,
	 * functionName);
	 * 
	 * final boolean isOverDepth = (currDepth < depth); // write texts (depth
	 * and number of samples) if needed
	 * dstp.paintOverDepthText(currSampleMidpoint, Math.min(succSampleMidpoint,
	 * attributes.numPixelsH), currDepth, functionName, isOverDepth,
	 * sampleCount); } }; detailPaint.paint(); }
	 */
	...
	private void setUpPaintDetailViewport(int linesToPaint,
			boolean changedBounds) {
		compositeOrigLines = new Image[linesToPaint];
		compositeFinalLines = new Image[linesToPaint];

		if (changedBounds) {
			final int num_traces = Math.min(attributes.numPixelsV,
					attributes.endProcess - attributes.begProcess);
